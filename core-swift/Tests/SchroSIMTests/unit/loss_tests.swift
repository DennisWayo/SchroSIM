...
final class LossTests: XCTestCase {
    ...
    func testCircuitRejectsInvalidLossEtaAtIRValidationTime() throws {
        let c = try Circuit(modes: 1)

        XCTAssertThrowsError(try c.append(.loss(eta: -0.01, mode: 0)))
        XCTAssertThrowsError(try c.append(.loss(eta: 1.01, mode: 0)))
        XCTAssertThrowsError(try c.append(.loss(eta: .infinity, mode: 0)))
    }
}
